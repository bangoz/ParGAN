...
class ImagePickleDataset(torch.utils.data.Dataset):
    def __init__(self, files, transforms=None):
        self.files = files
        self.transforms = transforms
...
def get_activations(files, model, batch_size=50, dims=2048, device='cpu', num_workers=8):
    ...
    dataset = ImagePickleDataset(files, transforms=TF.Normalize([-1.0], [2.0]))
...
def compute_statistics_of_path(path, model, batch_size, dims, device, num_workers=8):
    if path.endswith('.npz'):
        ...
        if type(files)==tuple: # real data
            files = files[0]
            files = files/files.max() * 2. - 1.
            files = files[random.sample(range(files.shape[0]), 1000)]
