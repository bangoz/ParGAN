    ...
    for i, (imgs, _) in enumerate(dataloader):
        ...
        if i % opt.n_critic == 0:
            ...
            if batches_done % opt.sample_interval == 0:
                save_image(fake_imgs.data[:25], "{}/images/{}.png".format(filepath, batches_done), nrow=5, normalize=True)

            batches_done += opt.n_critic

            if epoch % 20 == 0:
                z = Variable(Tensor(np.random.normal(0, 1, (opt.fid_batch_size, opt.latent_dim))))
                gen_imgs = generator(z).detach()
                torch.save(gen_imgs, "{}/images_for_fid/{}.pt".format(filepath, epoch))

info = {'d_losses': d_losses, 'g_losses': g_losses}
